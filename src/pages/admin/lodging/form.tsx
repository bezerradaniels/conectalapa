...
import type { LodgingWithRelations } from '@/types'
...
const schema = z.object({
  ...
  lodging_type: z.enum(['hotel', 'pousada', 'guesthouse', 'resort', 'other']),
...
export default function AdminLodgingFormPage() {
  const { id } = useParams()
  const isEditing = Boolean(id)
  const navigate = useNavigate()

  const { data: existing, isLoading: isLoadingExisting } = useLodgingAdminDetail(id)
  ...
  if (existing && !hydrated) {
    ...
    reset({
      ...
      lodging_type: existing.lodging_type as FormValues['lodging_type'],
  ...
  const lodgingTypeValue = watch('lodging_type')
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (saveIntentRef.current === 'new') {
      navigate('/admin/hospedagem/novo', { replace: true })
      window.location.reload()
    } else {
      navigate('/admin/hospedagem')
    }
  }

  const previewEntity = useMemo<LodgingWithRelations | null>(() => {
  ...
  return (
    <div>
      <Head title={isEditing ? 'Editar hospedagem' : 'Nova hospedagem'} />
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        <AdminFormShell
          title={isEditing ? existing?.name || 'Editar hospedagem' : 'Nova hospedagem'}
          backTo="/admin/hospedagem"
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              <GalleryUploadField label="Galeria de fotos" bucket="galleries" domain="lodging" entityId={entityId} value={gallery} onChange={setGallery} session={session} />
            </div>
      ...
      {showPreview && previewEntity && (
        <Dialog isOpen onClose={() => setShowPreview(false)} size="lg" title="Pré-visualização">
          <div className="max-h-[70vh] overflow-y-auto -mx-6 px-6">
            <LodgingDetailView lodging={previewEntity} related={[]} />
          </div>
        </Dialog>
      )}
    </div>
